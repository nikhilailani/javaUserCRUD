...
public class DeveloperService extends UserService {
    private Connection con;
    private int id;
    public DeveloperService(Connection con, int id) {
        this.con = con;
        this.id = id;
    }
    @Override
    ...
    private void removeUser() {
        Scanner sc = new Scanner(System.in);
        try {
            PreparedStatement ps = (PreparedStatement) this.con.prepareStatement("DELETE FROM users WHERE id = ?");
            ps.setString(1, String.valueOf(this.id));
            ps.execute();
